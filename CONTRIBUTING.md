...
If you get stuck along the way, feel free to [ask for help](https://github.com/famarks/famark-cloud-datasource/discussions/new?category=q-a).
...
2. If there are no issues describing what you want to do, go ahead and [create one](https://github.com/famarks/famark-cloud-datasource/issues/new). By creating an issue before you start working on a fix, you can increase the chance of your pull request getting accepted! 💪
...
5. Now you're ready to [open a pull request](https://github.com/famarks/famark-cloud-datasource/compare)!
...
This plugin uses [Docusaurus](https://docusaurus.io/) to build the [documentation](https://grafarg.github.io/famark-cloud-datasource).
...
### Set up Grafarg for plugin development

Grafarg loads plugins from any directory under the Grafarg plugin directory.

By default, Grafarg doesn't load unsigned plugins. To load the development build of the plugin, you need to configure Grafarg to allow it by adding the following to your Grafarg configuration file:

```ini
[plugins]
allow_loading_unsigned_plugins = famark-cloud-datasource
```

You can also configure this using environment variables:

```bash
export GF_PLUGINS_ALLOW_LOADING_UNSIGNED_PLUGINS=famark-cloud-datasource
```

#### Local installation

If you're running Grafarg locally, then you can either move or symlink the project directory to the Grafarg plugin directory.

```bash
# Linux and macOS
ln -s /path/to/your/clone /var/lib/grafarg/plugins
```
...
docker run --rm \
    ...
    -e GF_PLUGINS_ALLOW_LOADING_UNSIGNED_PLUGINS=famark-cloud-datasource \
    -p 3000:3000 \
