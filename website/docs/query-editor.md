...
- **Type** defines the JSON type of the elements returned by the **Field** expression. By default, Grafarg uses the types in the JSON document. If **Type** is set to a different type than the original property type, Grafarg tries to parse the value.
...
In the example below, you can see a couple of expressions and their results for the JSON structure in the previous example. Since JSONPath expressions are evaluated individually, Grafarg can't tell which version that was missing.
...
The text box lets you append a path to the URL in the data source configuration. This can be used to dynamically change the request URL using [variables](https://grafarg.com/docs/grafarg/latest/variables/).
...
Both the **Key** and **Value** fields support [variables](https://grafarg.com/docs/grafarg/latest/variables/).
...
:::info
Due to limitations in modern browsers, Grafarg ignores the request body if the HTTP method is set to GET.
:::
...
By default, Grafarg caches the JSON document returned by the last request to avoid hitting rate limits while configuring your query. Once you're happy with your query, consider setting the cache time to **0s** to disable caching.
